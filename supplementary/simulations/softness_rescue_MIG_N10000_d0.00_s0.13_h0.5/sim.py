...
nreps = 100 #number of replicates per m value
ms = [10**(i/100) for i in range(-185,1,20)] #m values to explore
...
for j,m in enumerate(ms):
	for i in range(nreps):
		...
		initialize() {
			...
			m1.mutationStackPolicy = "l"; //keep first mutation
		...
		early() {
			...
			if (m>0 & N>0) { //if adapting from migration and some offspring made
                                ...
                                for (i in target) {
                                        i.addNewDrawnMutation(m1, 0); //add the migrant allele
                                }
                        }
			...
			if ((u==0 & m==0 & freq == 0) | (N==0)) { //if fail to adapt
		        ...
		        if (freq == 1.0 & N == N0) { //if mutation fixed and population recovered
		            ...
		            writeFile("data/nalleles.csv", paste(c(m, n), ","), append=T);
