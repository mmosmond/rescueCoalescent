...
ks = [i for i in range(11,101+1,20)] #kvalues to explore
...
for j,k in enumerate(ks):
	for i in range(nreps):
		...
		early() {
			...
			//homos = sum(asInteger(off.countOfMutationsOfType(m1)>1));
			//catn(sim.generation + " " + N + " " + freq + " " + n + " " + homos);
			//writeFile("data/dynamics_k" + k + "_rep%d.csv", paste(c(sim.generation, N, freq, n, homos), ","), append=T);
			if ((u==0 & m==0 & freq == 0) | (N==0)) { //if fail to adapt
				catn("all hope was lost in generation " + (sim.generation-1)); //alert the user
				writeFile("data/nestablish_k" + k + ".csv", "0", append=T); //record number that establish
				sim.simulationFinished(); //end simulation
			}
			else {           
		        if (freq > 0.9 & N == N0) { //if mutation fixed and population recovered
					catn("rescue complete in generation " + (sim.generation-1.0));
		            //catn(n + " mutations contributed");
					writeFile("data/nestablish_k" + k + ".csv", asString(n), append=T); //record number that establish
