...
L = 3 #number of sites on chromosome
L0 = round(L/2) #location of selected site (one of the center sites)
u = 0 #mutation rate at selected site
m = 0 #migration rate
rbp = 0 #per basepair recombination rate
k = 5 #initial number of beneficial alleles
...
for i in range(nreps): 
	...
	1:%d early() {
		...
		if ((u==0 & m==0 & freq == 0) | (N==0)) { //if fail to adapt
	        ...
	        if (freq == 0.5 & N == K) { //if mutation fixed and population recovered
