...
ks = [i for i in range(1,101+1,20)] #kvalues to explore
	...
	for i in range(nreps):
		...
		early() {
			...
			if ((u==0 & m==0 & freq == 0) | (N==0)) { //if fail to adapt
				...
				writeFile("data/prescue_k" + k + ".csv", "0", append=T); //record extinction
		        ...
		        if (freq > 0.9 & N == N0) { //if mutation fixed and population recovered
					...
					writeFile("data/prescue_k" + k + ".csv", "1", append=T); //record rescue
