...
h = 0.5 #dominance
B = 2 #number of matings/offspring
...
for i in range(nreps):
	...
	early() {
		...
		if (m>0 & N>0) { //if adapting from migration and some offspring made
			target = sample(off.genomes, rpois(1,m)); //choose chromosomes to add a migrant allele to
	 		for (i in target) {
	 			if (i.countOfMutationsOfType(m1) == 0) { //if doesnt already have the migrant allele
	 				i.addNewDrawnMutation(m1, L0); //add the migrant allele
	 			}
			}
		}
	...
	X[i] = len(np.unique([i.genotypes for i in ts.variants()][0])) #number of unique copies of beneficial allele
	ts = msprime.mutate(ts.recapitate(rbp, Ne=N0).simplify(), rate=U, keep=True) #recapitate and overlay neutral mutations
	...
	#meanH = [i * (1-1/nsamples) for i in site_div] #convert diversity to heterozygosity
	...
	# save to file
	#csvData = zip(recombination, meanH, tajimasD)
	csvData = zip(recombination, site_div)
