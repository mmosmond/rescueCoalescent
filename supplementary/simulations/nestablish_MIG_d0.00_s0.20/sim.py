...
ms = [10**(i/10) for i in range(-19,1,2)] #migration rates to explore
datadir = "data" #location to put output files
k = 0 #initial number of beneficial alleles
nreps = 100 #number of replicates per m value (number of runs where allele fixes and population recovers)
maxt = 10000 #maximum number of generations (safestop that should never be reached)
...
for m in ms:
	...
	for i in range(nreps):
		...
		initialize() {
			...
			defineConstant("outfile_nestablish", "%s/nestablish_m%.3f.txt"); //where to save dynamics
		...
		""" %(N0,d,s,h,B,u,m,k,datadir,m,maxt,maxt+1)
