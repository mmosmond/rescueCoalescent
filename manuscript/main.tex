...
%\textcolor{red}{no longer use this equation so could move to sup mat}
%
...
The variance, $v$, depends on particulars of the lifecycle (see \nameref{sec:pestNe} for details).
...
%When $d\geq0$ the population remains at carrying capacity and the growth rate of the establishing heterozygote only depends on its relative fitness, $\epsilon = W_{Aa}/W_{aa} - 1 = (1 + s h)-1 = s h$, which is independent of demography, $d$.
...
%Note that $\epsilon$ does not depend on demography, $d$, in contrast to the growth rate of the heterozygote at establishment in a declining population (see \nameref{sec:initial_frequency}).
%The variance in the number of offspring, $v$, may also be different at the time of fixation than it was at the time of establishment (see \nameref{sec:pestNe}).
%\textcolor{red}{move to sup mat?}
...
\end{aligned}
\end{equation} 
where $N_e'(\tau)$ is the effective population size $\tau$ generations before fixation (see \nameref{sec:pestNe} for details).
...
Briefly, the lifecycle described in \nameref{sec:lifecycle} was simulated in \texttt{SLiM} 3 \citep{Haller2019slim} with tree-sequence recording \citep{Haller2019tree}.
...
In \nameref{sec:sweep+bottle_sup} we compare rescue, with expected effective population size $N_r$ (from Equation \ref{eq:ptnt}), to sweeps that occur in populations suddenly bottlenecked to size $N_r$.
...
%Integrating the expected number of copies of the allele, $e^{(t-\tau) \epsilon} / P_\mathrm{est}$, over the distribution of waiting times shows that the waiting time factor in a population of roughly constant size is $4N(0)u/(1+4N(0)u)$ \citep[\textcolor{red}{not incidentally the expected heterozygosity of an ideal population},][]{kimura1964number,watterson1975number}, which, in contrast to rescue, depends strongly on mutational input but is independent of establishment probability.
...
In \nameref{sec:extra_mig} we derive the expected backward-time dynamics and structured coalescent under rescue by migration.
...
We chose to use this setup as the constant population size comparison (rather than, say, a Wright-Fisher population) because it allows us to keep the same variance in gamete numbers (affecting the probability of establishment and the rate of coalescence; see \nameref{sec:pestNe}) as well as the same census population size (affecting the initial and effective allele frequencies) as in the case of rescue.
...
In \nameref{sec:rescue_forward_SGV} we have compared the genetic signatures of rescue to those from sweeps in populations of constant size. 
...
\end{tikzpicture}
\caption{
Life-cycle and time notation. The arrows indicate the forward-time direction and the numbers indicate the time before fixation (i.e., starting in generation $\tau$ and moving forward in time through $\tau-1/5$, $\tau-2/5$, ... we arrive at generation $\tau-1$, one generation closer to fixation).
}%
\label{fig:lifecycle}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\node[above = 0 of A, darkyellow, draw, rectangle, rounded corners, thick, fill=darkyellow!25] (N) {\fontfamily{phv}\selectfont \textbf{coincident}};
...
\node[below = 0 of R, darkyellow, draw, rectangle, rounded corners, thick, fill=darkyellow!25] (Bot) {\fontfamily{phv}\selectfont \textbf{coincident}};
